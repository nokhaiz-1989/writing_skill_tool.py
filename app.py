            ...
            if not df['Valid_ID'].all() or not df['Valid_Score'].all():
                ...
                if post_file:
                    ...
                    if 'ID' in post_df.columns and 'Score' in post_df.columns:
                        ...
                        shift_matrix = shift_matrix.reindex(index=SEGMENT_LABELS, columns=SEGMENT_LABELS, fill_value=0)
                        fig5, ax5 = plt.subplots(figsize=(6, 2))
                        cmap = sns.light_palette("blue", n_colors=5, input="xkcd")
                        sns.heatmap(shift_matrix, annot=True, fmt='d', cbar=False, cmap=cmap, ax=ax5,
                                    ...
                                    linewidths=0.5, linecolor='gray', annot_kws={"fontsize": 6})
